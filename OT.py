...
def GW_distance(X, Y, p, q, lamda=0.5, iteration=5, OT_iteration=20,C = None, alpha=0.5):
	'''
	:param X, Y: M x bs x d; N x n_cls x d
	:param p, q: probability vectors
	:param lamda: regularization
	:return: GW distance
	'''
	Cs = cos_batch_torch(X, X).float().cuda()	## bs x M x M;
	Ct = cos_batch_torch(Y, Y).float().cuda()   ## n_cls x N x N;
	# pdb.set_trace()IPOT_distance_torch_batch_uniform
	bs = Cs.size(0)
	n_cls = Ct.size(0)
	m = Ct.size(2)
	n = Cs.size(2)
	Cs = Cs.repeat(n_cls,1,1)
	Ct = Ct.repeat(bs,1,1)
	T, Cst = GW_torch_batch(Cs, Ct, bs*n_cls, n, m, p, q, beta=lamda, iteration=iteration, OT_iteration=OT_iteration,C=C, alpha=alpha)
	temp = torch.bmm(torch.transpose(Cst,1,2), T)
	distance = batch_trace(temp, m, bs*n_cls)
	return distance, T

def GW_torch_batch(Cs, Ct, bs, n, m, p, q, beta=0.5, iteration=5, OT_iteration=20,C=None,alpha=0.5):
	'''
	C is for cross modality cost function,
	If C is not none, we use a shared OT to optimize both WD and GWD；
	Returns:
 	- gamma: overall transport matrix
 	- Cgamma: overall distance;
	'''
	...
	for i in range(iteration):
		if C is not None:
			C_gamma = alpha*(Cst - 2 * torch.bmm(torch.bmm(Cs, gamma), torch.transpose(Ct, 1, 2))) + (1.0-alpha)* C
	...
	if C is not None:
		Cgamma = alpha * (Cst - 2 * torch.bmm(torch.bmm(Cs, gamma), torch.transpose(Ct, 1, 2))) + (1.0-alpha)* C
	else:
...
def GW_distance_uniform(X, Y, lamda=1e-1, iteration=5, OT_iteration=20,C=None,alpha=0.5):
	m = X.size(2)
	n = Y.size(2)
	bs = X.size(0)
	n_cls = Y.size(0)
	p = (torch.ones(bs*n_cls, m, 1)/m).cuda()
	q = (torch.ones(bs*n_cls, n, 1)/n).cuda()
	return GW_distance(X, Y, p, q, lamda=lamda, iteration=iteration, OT_iteration=OT_iteration, C=C,alpha=alpha)
