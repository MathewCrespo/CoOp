...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        wd, T_wd = IPOT_distance_torch_batch_uniform(cos_distance, bs*n_cls, m, n, 30) ## pay attention to the order of m,n here
        
        ## calculate Gromov Wasserstein Distance
        #image_features2 = image_features.repeat(n_cls,1,1)
        #text_features2 = text_features.repeat(bs,1,1)
        gwd, T_gwd = GW_distance_uniform(image_features.permute(1,2,0), text_features.permute(1,2,0))
        
        


        sim_op = torch.sum(T_wd * sim, dim=(1, 2)) ## (BC)
        sim_op = sim_op.contiguous().view(b,self.n_cls)
        
        sim_op2 = torch.sum(T_gwd * sim, dim=(1, 2)) ## (BC)
        sim_op2 = sim_op2.contiguous().view(b,self.n_cls)

        logit_scale = self.logit_scale.exp()
        #logits = logit_scale * image_feature_pool @ text_feature_pool.t()
        logits = logit_scale * sim_op
        logits2 = logit_scale * sim_op2
        
        # logits2 = (logits2 + logits)
        if self.ensemble == 'gwd':
            return logits2
