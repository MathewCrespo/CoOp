...
class CustomCLIP(nn.Module):
    ...
    def feature_select_sim_per(self,ref,feature_maps,p=0.9):
        '''
        ref: pooled feature vector (bxd)
        feature map: (Mxbxd)
        '''
        M,d = feature_maps.shape[0],feature_maps.shape[2]
        n = int(p*M)
        sim = torch.einsum('bd,mbd->mb',[ref,feature_maps]) ## M b
        
        values, indices = sim.topk(n,dim=0)
        
        indices = indices.unsqueeze(-1).expand(-1,-1,d)
        feature_maps = torch.gather(feature_maps,dim=0,index=indices)
    ...
    def forward(self, image):
        ...
        image_features = self.feature_select_sim_per(image_feature_pool, image_features)
        M = image_features.shape[0]

        sim = torch.einsum('mbd,ncd->mnbc', image_features, text_features).contiguous()  # M N B C
        sim = sim.view(M,self.N,b*self.n_cls) ## Batch size and class number view together
        ...
        with torch.no_grad():
            KK = torch.exp(-wdist / self.eps)  ## (BC) M N
            T = self.Sinkhorn(KK,xx,yy) ## (BC) M N
