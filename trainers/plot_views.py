...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        b = image.shape[1]        
        image_pool = []
        for image_view in image:
            image_features = self.image_encoder(image_view.type(self.dtype))[0]
            image_pool.append(image_features)
        
        image_features = torch.stack([x for x in image_pool],dim=0) ## 
...
class PLOT_Views(TrainerX):
    ...
    def parse_batch_train(self, batch):
        input = batch["img"]
        label = batch["label"]
        input = torch.stack([img for img in input])
