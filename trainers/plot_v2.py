...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        ### sim is used for intra-image W distance
        ...
        sim2 = sim.view(M*self.N, b, self.n_cls) ## (MN), B, C
        ...
        xx2=torch.zeros(M*self.N, b, dtype=sim2.dtype, device=sim2.device).fill_(1. / b)
        yy2=torch.zeros(M*self.N, self.n_cls, dtype=sim2.dtype, device=sim2.device).fill_(1. / self.n_cls)
        ...
        with torch.no_grad():
            ...
            KK2 = torch.exp(-wdist2 / self.eps) #(MN), B, C
            T2= self.Sinkhorn(KK2,xx2,yy2)# (MN), B, C
        ...
        sim_op2 = torch.sum(T2 * sim2, dim=0)
        ...
        logits3 = logit_scale * sim_op2
        # logits2 = (logits2 + logits)
        return (logits2 + logits3)/2


@TRAINER_REGISTRY.register()
class PLOTV2(TrainerX):
