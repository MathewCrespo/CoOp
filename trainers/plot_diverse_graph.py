...
import sys
sys.path.append("..")
from OT import GW_distance_uniform,GW_distance,cost_matrix_batch_torch, IPOT_torch_batch_uniform, GW_distance_uniform, IPOT_distance_torch_batch_uniform
import numpy as np
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        image_feature_pool = image_features[0] # b x 1024
        image_features = image_features[1:]  # M x b x 1024 M=49 here
        M = image_features.shape[0]
        a = int(np.sqrt(M))
        self.d = image_features.shape[-1]
        
        # hierachical feature map pooling:
        image_featuremap = image_features.view(a,a,b,self.d).permute(2,3,0,1) ## b,d,7,7
        image_pool1 = self.pooling1(image_featuremap).view(b,self.d,-1).permute(2,0,1) # M1,b,d
        image_pool2 = self.pooling2(image_featuremap).view(b,self.d,-1).permute(2,0,1) # M2,b,d
        
        image_features = torch.cat([image_features,image_pool1,image_pool2],dim=0) # M' b d

        prompts = self.prompt_learner()   # b x 1024 (400,77,512)
        tokenized_prompts = self.tokenized_prompts
        text_features = self.text_encoder(prompts, tokenized_prompts) # (N x 100) x 1024
        text_features =  text_features.contiguous().view(self.N, self.n_cls, self.d)  # N c d=1024 (4 100 1024)
        ...
        ### Graph Match begins here
        '''
        image features: M x b x 1024 
        text features: N x n_cls x 1024
        '''
        bs, n_cls, m, n = image_features.shape[1], text_features.shape[1], image_features.shape[0], text_features.shape[0]
        #cos_distance = cost_matrix_batch_torch(image_features.permute(1,2,0), text_features.permute(1,2,0))
        ...
        sim = sim.view(m,n,bs*n_cls) ## Batch size and class number view together
        sim = sim.permute(2,0,1)  ## (BC),M,N . (6400,49,4)
        cos_distance = 1.0 - sim
        ## calculate  WD and GWD together
        distance, T = GW_distance_uniform(image_features.permute(1,2,0), text_features.permute(1,2,0),C=cos_distance,alpha=self.alpha)
        
        


        sim_op = torch.sum(T * sim, dim=(1, 2)) ## (BC)
...
class PLOT_Diverse_Graph(TrainerX):
